...
integer, save :: i_qmain = 0 ! counter number of calls of XNL_MAIN
integer, save :: i_qlast = 0 ! value of iquad in last call
...
!------------------------------------------------------------------------------
  ...
  if(iq_err/=0) exit q_cmplocus_flow
...
if(nlocusx==0) exit q_getlocus_flow
...
!-----------------------------------------------------------------------------
      ...
      if(iq_err/=0) exit q_makegrid_flow
!------------------------------------------------------------------------------
...
exit q_makegrid_flow
end do q_makegrid_flow
...
if(ik1==ik3 .and. ia1==ia3) exit q_t13v4_flow  ! skip routine if k1=k3
...
!  only active when IQ_SEARCH==1 .AND. IQ_TYPE==3
  ...
  if(iq_err /=0) exit q_xnl4v4_flow
end if
...
!     09/10/1999 Values of w2 and w4 in REAL(KIND=KIND(0.0D0))
...
real e,g,gsq,omsq23
real, parameter :: pi4 = 0.785398163
real dot123,dot23
!!real cosz,tanz
...
REAL(KIND=KIND(0.0D0)) wsqp12         ! derived variable
REAL(KIND=KIND(0.0D0)) wsqm13         ! derived variable
REAL(KIND=KIND(0.0D0)) wsq13          ! derived variable
REAL(KIND=KIND(0.0D0)) wsqm14         ! derived variable
REAL(KIND=KIND(0.0D0)) wsq14          ! derived variable
REAL(KIND=KIND(0.0D0)) wsq12          ! derived variable
