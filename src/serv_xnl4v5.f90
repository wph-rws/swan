...
!-----------------------------------------------------------------------
REAL(KIND=KIND(0.0D0)), PARAMETER :: EPS=3.d-14
INTEGER i,j,m
REAL(KIND=KIND(0.0D0)) p1,p2,p3,pp,xl,xm,z,z1
!-----------------------------------------------------------------------
      m=(n+1)/2
      xm=0.5d0*(x2+x1)
      xl=0.5d0*(x2-x1)
      do i=1,m
        ...
        w(i)=real(2.d0*xl/((1.d0-z*z)*pp*pp))
        w(n+1-i)=w(i)
      end do
...
logical finished                  ! iteration reached a terminal condition
