...
!   Source text
          ...
          if ( BGRIDP(6*ib-4) == 1 ) then
             ...
             if ( NPTST > 0 ) then
                do j = 1, NPTST
                   ...
                   if ( lpb ) then
                      if ( OPTG /= 5 ) then
                         write (PRTEST,"(' boundary point', 3i8)") ib, ix-1, iy-1
                      else
                         write (PRTEST,"(' boundary vertex', 2i8)") ib, ix
                      endif
                      write (PRTEST,"(' mean wave number and standard deviation: ', 2e12.4)") km, sd
                   endif
                enddo
             endif
    ...
    call SWREDUCE( kmean, 1, SWMAX )
    call SWREDUCE( sdev , 1, SWMIN )
    call SWREDUCE( hsi  , 1, SWMAX )
    if ( stpnow() ) return
    ...
    if ( .not. kxlen /= 0. .and. .not. kylen /= 0. ) then
       ...
       ! perform global reductions in parallel run
       call SWREDUCE( hmin, 1, SWMIN )
       call SWREDUCE( hmax, 1, SWMAX )
       if ( stpnow() ) return
    ...
    if ( dkx > sdev .or. dky > sdev ) then
       ...
       write (PRINTF, "(' maximum resolvable coherent length scale is ', f9.2,' m')") PI2 / max(dkx,dky)
    endif
    ...
    if (SCREEN /= PRINTF ) then
       write (PRINTF,"(/' preparing data for quasi-coherent modelling:'/ ' - maximum resolvable coherent length scale = ',f9.2,' m'/ ' - number of Fourier components = ', i4/ ' - xmin = ',f9.2, ', xmax = ',f9.2, ', ymin = ',f9.2, ', ymax = ',f9.2,' (m)'/ ' - qxmin = ',f9.3, ', qxmax = ',f9.3, ', qymin = ',f9.3, ', qymax = ',f9.3,' (rad/m)')") clen, ncoz, xpsc(1), xpsc(ncoz)+dxps, xpsc(1), xpsc(ncoz)+dxps, -PI/dxps, PI/dxps, -PI/dxps, PI/dxps
       if (IAMMASTER .and. IGEN == 4 ) write (SCREEN,"(' ... preparing data for quasi-coherent modelling'// ' maximum resolvable coherent length scale = ',f9.2,' m'/ ' number of Fourier components = ', i4/)") clen, ncoz
    endif
...
!   Local variables
    ...
    REAL(KIND=KIND(0.0D0))                       :: area     ! twices the area of centroid dual around present vertex
    real                         :: cg       ! group velocity in wave number space
    real                         :: ctb      ! contribution term
    real                         :: dir      ! spectral directions in wave number space
    ...
    REAL(KIND=KIND(0.0D0))                       :: x0       ! x-coordinate of the centroid of present cell
    REAL(KIND=KIND(0.0D0))                       :: x1       ! x-coordinate of the centroid of next cell
    REAL(KIND=KIND(0.0D0))                       :: y0       ! y-coordinate of the centroid of present cell
    REAL(KIND=KIND(0.0D0))                       :: y1       ! y-coordinate of the centroid of next cell
...
!   Source text
    ...
    do jc = 1, vert(ivert)%noc
       ...
       if ( dloc(1) <= DEPMIN .or. dloc(2) <= DEPMIN .or. dloc(3) <= DEPMIN ) then
          W = 0.
          dwdx = 0.
          dwdy = 0.
          return
       ...
       do iky = 1, mkyc
          ...
          do ik = 0, min(ikx-1,ish-1)
             do jk = 0, min(iky-1,ish-1)
                ...
                ctb = ctb + 2. * ( sig + kxl*AIMAG(u) + kyl*AIMAG(v) ) * W(iky-jk,ikx-ik) - cax * dwdx(iky-jk,ikx-ik) - cay * dwdy(iky-jk,ikx-ik)
