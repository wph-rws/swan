...
!   Source text
    ...
    call SWREDUCE( hsmean, 1, SWSUM )
    call SWREDUCE( tmmean, 1, SWSUM )
    ...
    do ivert = 1, nverts
       ...
       if ( vert(ivert)%active ) then
          ...
          if (tstfl) then
             if (lhead) write(PRINTF,"(13x,'dHrel ','dHoval ','dTm01rel ','dTm01oval ')")
             write (PRINTF,"(1x,ss,'k=',i7,' ',1pe13.6e2,' ',1pe13.6e2,' ',1pe13.6e2,' ',1pe13.6e2)") ivert, hsabs/hsprev, hsabs/hsmean, tmabs/tmprev, tmabs/tmmean
             lhead = .false.
          endif
