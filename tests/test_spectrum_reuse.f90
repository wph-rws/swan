program test_spectrum_reuse
!  Seriele herbruikbaarheid bij wisselende spectrumafmetingen.
!  A draait met CIRCLE 12 0.05 1.0 8, B met CIRCLE 16 0.05 1.0 10 (zelfde grid,
!  zelfde fysica GEN3 KOMEN), daarna A opnieuw, alle drie in een proces. De
!  tweede A moet bit-voor-bit gelijk zijn aan de eerste (geen spectrumtoestand
!  die van B is blijven hangen: MSC/MDC, spectrale tabellen via rebuild,
!  brontermwerkruimten en uitvoerlijsten), terwijl B aantoonbaar anders heeft
!  gerekend dan A. Iedere SWMAIN-terugkeer moet bovendien alle eigenaars leeg
!  hebben achtergelaten, net als in test_two_cases.
!
!  Dit bewijst uitdrukkelijk nog geen twee tegelijk bestaande instanties en
!  geen threadveiligheid.
   ...
   if (all(transfer(first, [1, 1, 1, 1]) == &
      ...
      error stop "the contrasting spectrum did not change the answer"
   end if

   call SWEXITMPI
   print *, 'A-B-A spectrum runs agree and state is clear'

contains

   subroutine run_once(fine_spectrum, values, ierr)
      logical, intent(in)     :: fine_spectrum
      ...
      open(newunit=unit, file='spectrum_reuse.tbl', status='old', iostat=dummy)
      if (dummy == 0) close(unit, status='delete')

      call write_deck(fine_spectrum)
      call SWMAIN
   ...
   subroutine read_table(values, ierr)
      ...
      values = 0.0
      open(newunit=unit, file='spectrum_reuse.tbl', status='old', action='read', &
           iostat=ierr)
      if (ierr /= 0) return
   ...
   subroutine write_deck(fine_spectrum)
      logical, intent(in) :: fine_spectrum
      integer :: unit
      ...
      write(unit, '(A)') "TABLE 'CENTER' HEADER 'spectrum_reuse.tbl' XP YP " // &
...
end program test_spectrum_reuse
