#!/usr/bin/env python3
"""Build-independent runner for the SWAN diffraction test."""
...
def run(
    ...
    shutil.copyfile(case_directory / "diffraction.swn", input_file)
    ...
    if print_file.exists():
        print_file.replace(case_directory / "diffraction.prt")
    error_file = case_directory / "Errfile"
    if error_file.exists():
        error_file.replace(case_directory / "diffraction.erf")

    if result.returncode:
        raise RuntimeError(
            f"SWAN stopped with exit code {result.returncode}. "
            "Inspect diffraction.prt and diffraction.erf."
        )
    if not (case_directory / "norm_end").is_file():
        raise RuntimeError(
            "SWAN did not create norm_end. Inspect diffraction.prt and diffraction.erf."
        )

    reference_directory = Path(__file__).resolve().parent / reference_name
    if smoke:
        compare_with_reference(case_directory, reference_directory,
                               ("diffraction_center.tbl", "diffraction_hs.blk"),
                               smoke=True)
    ...
    print(f"Diffraction test completed normally in {elapsed:.2f} seconds.")
    print("Results: diffraction_center.tbl, diffraction_hs.blk, diffraction.prt")
    if elapsed > 180:
...
def main() -> int:
    ...
    try:
        ...
        if case_directory != source_directory:
            ...
            for name in ("diffraction.swn", "bottom.bot"):
